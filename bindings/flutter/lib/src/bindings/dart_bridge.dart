...
///
/// 1. Two ABI callback typedefs return int32_t (`flm_progress_callback`,
///    `flm_delta_callback`), but `NativeCallable.listener` only supports
///    void-returning callbacks. The trampolines wrap each with an
///    int32_t-returning C function that always returns 0; cancellation is
///    expressed through `flm_job_cancel`.
///
...
///    stack. The shim therefore deep-copies the payload synchronously and
///    hands the owned heap pointer to Dart; Dart releases it via the matching
...
final class DartBridge {
  ...
  static Pointer<
    ...
    return _progressAdapter ??= NativeLibrary.process.lookup<
        NativeFunction<
            Int32 Function(Uint64, Pointer<raw.flm_progress>,
  ...
  external Pointer<
      NativeFunction<
          Void Function(Uint64, Pointer<raw.flm_progress>,
              Pointer<Void>)>> on_progress;

  external Pointer<
      NativeFunction<
          Void Function(
              Uint64, Pointer<raw.flm_delta>, Pointer<Void>)>> on_delta;

  external Pointer<NativeFunction<raw.flm_completion_callback_native>>
      on_complete;
